# Introduction
Want to try out SDV (Software Defined Vehicles) without installing lots of things? The vehicle-edge-runtime Docker container has everything you need to run your QM apps. It's made for people who are new to SDV and want to learn and practice without a complicated setup.
The `vehicle-edge-runtime` connects natively with [playground.digital.auto](https://playground.digital.auto) where you can ideation, coding and present your automotive feature.
...
```
docker run -d -e RUNTIME_NAME="MyRuntimeName" ghcr.io/eclipse-autowrx/vehicle-edge-runtime:latest

# With custom prefix
docker run -d -e RUNTIME_PREFIX="Kit-" -e RUNTIME_NAME="MyRuntimeName" ghcr.io/eclipse-autowrx/vehicle-edge-runtime:latest
```
...
```
docker run -d -e RUNTIME_NAME="MyRuntimeName" -p 55555:55555 ghcr.io/eclipse-autowrx/vehicle-edge-runtime:latest
```
...
```
docker run -d -e RUNTIME_NAME="MyRuntimeName" -e SYNCER_SERVER_URL="YOUR_SERVER" ghcr.io/eclipse-autowrx/vehicle-edge-runtime:latest
```

- Run your runtime with the local self manager. In this case everything stay in localhost, no external connection.
```
docker run -d -e RUNTIME_NAME="MyRuntimeName" -e SYNCER_SERVER_URL="http://localhost:3090" -p 3090:3090 ghcr.io/eclipse-autowrx/vehicle-edge-runtime:latest
```
...
docker buildx build --push --platform linux/amd64,linux/arm64 -t eclipse-autowrx/vehicle-edge-runtime:latest -f Dockerfile .
```
...
docker buildx build --platform linux/amd64 -t vehicle-edge-runtime:latest -f Dockerfile .
```
